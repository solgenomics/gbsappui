...
results_email () {
    ...
    if [[ ($run_gbsapp -eq 1) && ($run_beagle -eq 1)]]; then
        ...
        if [ $beagle_error -eq 1 ]; then
            ...
            $beagle_terminating_lines The imputation log file (if available) is attached. Please contact our developer team using the contact form at ${gbsappui_domain_name}/contact/form to resolve this error."
            if [ -f ${projdir}beagle/beagle_log.out ]; then
    ...
    # else
        ...
        until [ -f $beagle_output.vcf.gz ]; do
            ...
            if [ $(squeue -j $jobnum_gbsappui -h --noheader | awk '{n=split($0,a," ");print a[6]}' | awk '{n=split($0,a,":");print a[1]}' | awk '{n=split($0,a,":");print a[1]}' ) -gt 95 ]; then
                ...
                Full log files attached. Please contact our developer team using the contact form at ${gbsappui_domain_name}/contact/form to resolve this issue."
...
gbsapp () {
    ...
    if [ -f ${projdir}Analysis_Complete ]; then
        ...
        if [ -f ${projdir}snpcall/*x.vcf.gz ]; then
            ...
            Full log files also attached. Please contact our developer team using the contact form at ${gbsappui_domain_name}/contact/form to resolve this issue."
            error_email_gbsapp "$body"
            exit 0
        fi
...
if [ $run_beagle -eq 1 ]; then
    ...
    if [ $run_gbsapp -eq 0 ]; then
        if [ -f ${projdir}*vcf* ]; then
            ...
            body="Imputation for analysis ${analysis_name} did not complete successfully. The input vcf file to impute could not be found. Please contact our developer team using the contact form at ${gbsappui_domain_name}/contact/form to resolve this issue."
            error_email_beagle "$body"
            exit 0
        fi
    fi
    ...
    if [ $run_gbsapp -eq 1 ]; then
        ...
        #otherwise run it on the snpcall file
            ...
            body="Calling and filtering for analysis ${analysis_name} did not complete successfully. Please contact our developer team using the contact form at ${gbsappui_domain_name}/contact/form to resolve this issue."
            error_email_beagle "$body"
            exit 0
        fi
    fi
    ...
    if [ $beagle_error -eq 1 ]; then
        ...
        if [ $run_gbsapp -eq 1 ]; then
            ...
            $beagle_exception_lines
            $beagle_illegal_lines
            $beagle_terminating_lines Full log file also attached. Please contact our developer team using the contact form at ${gbsappui_domain_name}/contact/form to resolve this error."
            error_email_beagle "$body"
            exit 0
        fi
